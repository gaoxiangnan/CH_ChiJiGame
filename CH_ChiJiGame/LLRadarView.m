...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.frame = CGRectMake(self.center.x, self.center.y, frame.size.width, frame.size.height);
        [self buildingParams];
    }
    return self;
}
...
- (void)buildingParams {
    ...
    self.alphaColor = kRGBColor(212, 160, 74);
    self.circleCount = 3;
    self.circleColor = kRGBColor(212, 160, 74);
    self.circleWidth = 2.f;
    self.showCrossline = NO;
    self.crosslineColor = kRGBColor(212, 160, 74);
    self.crosslineWidth = 2.f;
    self.duration = 1.5f;
    self.needOpacityAnimation = NO;
    self.isExecute = NO;
    self.stopOpacity = YES;
    self.pointColor = kRGBColor(212, 160, 74);
